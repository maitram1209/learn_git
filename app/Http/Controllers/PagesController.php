...
class PagesController extends Controller
{
    public function productlist(Request $request){
        $mobile = DB::table('products')
                ...
                ->where('products.name', 'like', '%'.$request->key.'%', 'AND','category.parent_id','=','1')
        ...
        $lap = DB::table('products')
                ->join('category', 'products.cat_id', '=', 'category.id')
                ->join('pro_details', 'pro_details.pro_id', '=', 'products.id')
                ->where('products.name', 'like', '%'.$request->key.'%', 'AND','category.parent_id','=','2')
                ->select('products.*','pro_details.cpu','pro_details.ram','pro_details.screen','pro_details.vga','pro_details.storage','pro_details.exten_memmory','pro_details.cam1','pro_details.cam2','pro_details.sim','pro_details.connect','pro_details.pin','pro_details.os','pro_details.note')
                ->paginate(9);
        $pc = DB::table('products')
                ->join('category', 'products.cat_id', '=', 'category.id')
                ->join('pro_details', 'pro_details.pro_id', '=', 'products.id')
                ->where('products.name', 'like', '%'.$request->key.'%', 'AND','category.parent_id','=','19')
                ->select('products.*','pro_details.cpu','pro_details.ram','pro_details.screen','pro_details.vga','pro_details.storage','pro_details.exten_memmory','pro_details.cam1','pro_details.cam2','pro_details.sim','pro_details.connect','pro_details.pin','pro_details.os','pro_details.note')
                ->orderBy('products.created_at','desc')
                ->paginate(6);

    	return view('search',['mobile'=>$mobile,'laptop'=>$lap,'pc'=>$pc]);
    }
    ...
    {
        $ra = Rating::paginate(10);
        if ($cat =='mobile') {
            $mobile = Products::where('id',$id)->first();
            if (empty($mobile)) {
                return view ('errors.503');
                } else {
                   return view ('detail.mobile',['data'=>$mobile,'slug'=>$slug, 'ra'=>$ra]);
               }
        }
        elseif ($cat =='laptop') {
           ...
           return view ('detail.laptop',['data'=>$lap,'slug'=>$slug, 'ra'=>$ra]);
            }
        }
        elseif ($cat =='pc') {            
            ...
            if (empty($pc)) {
                ...
                return view ('detail.pc',['data'=>$pc,'slug'=>$slug, 'ra'=>$ra]);
            }
        } else {
            return redirect()->route('index');
        }
