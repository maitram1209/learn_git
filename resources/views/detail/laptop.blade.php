...
@section('content') 
<style>
  .rating_item .active{color: #ff9705 !important;}
  .rating_a .active{color: #ff9705 !important;}
</style>
    ...
    <div class="col-xs-12 col-sm-8 col-md-8 col-lg-8 no-padding">
      <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
              ...
              <!-- hot new content -->
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 no-padding">
                  ...
                  <div class="row">
                    ...
                    <div class="rating_a" style="font-size:40px; text-align:center">                       
                    <?php 
                     if($data->pro_total_rating>=1&&round($data->pro_total_number/$data->pro_total_rating,2)>1) {                   
                      for($i=1;$i<=5;$i++){                      
                      ?>
                        <i class="fa fa-star {{$i<=round($data->pro_total_number/$data->pro_total_rating,2) ? 'active' : ''}}" style="color: #999"></i>
          ...
          <div class="comment_rating" style="margin-bottom:20px">
            <h3 style="color:#777">Đánh giá</h3>
            <h5>Có tất cả {{$data->pro_total_rating}} đánh giá cho sản phẩm này</h5>
            <div class="comment_rating_content" style="display:flex;align-items:center; border: 1px solid #dedede; border-radius:5px">
              <div class="rating-item" style="width:20%;position:relative">
                  <span class="fa fa-star" style="font-size:100px;display:block;color:#ff9705;
                  margin:0 auto; text-align:center;"></span><b style="position: absolute;top:50%;left:50%;
                  transform:translateX(-50%) translateY(-50%);color:white;font-size:20px"><?php if($data->pro_total_rating>=1) { ?>
                  {!!round($data->pro_total_number/$data->pro_total_rating,2)!!} <?php } else { ?>0<?php } ?></b>
              </div>
              <div class="list_rating" style="width:60%;padding:20px;">
                ...
                <?php 
                  $a=0;
                  foreach($ra as $row){
                    if($row->ra_product_id==$data->id){
                      if($row->ra_number==$i){$a+=1;}
                    }
                  }                                  
                ?>
                <div class="item-rating" style="display:flex;align-items:center">
                  <div style="width:10%">
                    {{$i}} <span class="fa fa-star" style="color:#999"></span>
                  </div>
                  <div style="width:60%; margin: 0 20px;">
                    <span style="width:100%; height:10px;display:block;border:1px solid #dedede;border-radius:5px;background-color:#dedede">
                    <?php
                    if($data->pro_total_rating>=1) { ?>
                    <b style="width:{!!($a/$data->pro_total_rating)*100!!}%;background-color:#f25800;display:block;height:100%;border-radius:5px"></b>
                    <?php } else { ?>
              ...
              <div style="width:20%">
              @if (Auth::guest())
                    <a href="{{url('/login')}}" style="text-decoration: none;width:200px;background:#288ad6;padding:10px;color:white;border-radius:5px">Đăng nhập</a>
                @else
                <a href="#" class="js_rating_action" style="text-decoration: none;width:200px;background:#288ad6;padding:10px;color:white;border-radius:5px">Gửi</a>
                @endif
              </div>              
            </div>
            <style>
            ...
            <div class="form_rating hide">
              ...
              <div style="margin-top: 15px;">
                <a href="{{route('post.rating.product',$data)}}" class="js_rating_product" style="text-decoration: none;width:200px;background:#288ad6;padding:10px;color:white;border-radius:5px">Gửi</a>
              </div>
            </div>
          </div>          
          <div class="comment_list_rating" style="height:300px;padding:20px; border: solid 1px #dedede; border-radius:5px;overflow-x:hidden;overflow-y:auto;">
            @foreach($ra as $row)
            @if($row->ra_product_id==$data->id)
            <div class="rating_item">
              <div>
                <span style="color:#333; font-weight:bold;text-transform:capitalize;">{!!$row->user->name!!} - </span>
                <a href="" style="color:#2ba832"><i class="fa fa-check-circle"></i> Đã mua hàng tại website</a>
                <p>
                  <span>
                  <?php 
                     if($data->pro_total_number&&round($data->pro_total_number/$data->pro_total_rating,2)>1) {                   
                      for($i=1;$i<=5;$i++){                      
                      ?>
                        <i class="fa fa-star {{$i<=round($data->pro_total_number/$data->pro_total_rating,2) ? 'active' : ''}}" style="color: #999"></i>
                      <?php } }else { for($i=1;$i<=5;$i++){?>
  ...
  $(document).ready(function() {
    ...
    $('.js_rating_action').click(function (e) {
      ...
      if($('.form_rating').hasClass('hide')){
        ...
        $('.js_rating_action').text('Ẩn');
      }
      else{
        $('.form_rating').addClass('hide').removeClass('active');
        $('.js_rating_action').text('Gửi');
      }
